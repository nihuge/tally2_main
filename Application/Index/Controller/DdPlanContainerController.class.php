...
class DdPlanContainerController extends BaseController
{
	...
	public function add($plan_id)
	{
		...
		if(I('post.CTNNO') and I('post.CTNSIZE') and I('post.CTNTYPE') and I('post.SEALNO'))
		{
			...
			//箱型
			$ctn_size = $container->field('ctn_size')->group('ctn_size')->select();
			$this->assign('ctn_size',$ctn_size);
			//尺寸
			$ctn_type = $container->field('ctn_type')->group('ctn_type')->select();
			$this->assign('ctn_type',$ctn_type);
