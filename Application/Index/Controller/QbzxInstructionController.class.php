...
class QbzxInstructionController extends BaseController
{
	...
	public function index()
	{
		...
		$list = $plan
			->alias('p')
			->field("p.*,s.ship_name,l.location_name,c.customer_shortname as customer,c.customer_name")
			->where("p.id!=''")
			->join("left join tally_ship s on p.ship_id=s.id")
			->join("left join tally_location l on p.location_id=l.id")
			->join('left join tally_customer c on p.entrust_company=c.id')
			->limit($begin,$per)
			->order('p.id desc')
			->select();
		$num=count($list);
		...
		{
			//已配箱数
			$where = "c.status!='-1' and i.plan_id='".$v['id']."'";
			$has_container_num = $instruction
						->alias('i')
						->field("i.id")
						->join("left join tally_qbzx_instruction_ctn c on c.instruction_id=i.id")
						->where($where)
						->count();
			$list[$key]['has_container_num'] = $has_container_num;
			//获取指令状态,判断预报下的指令是否都已经完成，都已完成颜色变红
			$where1 = " plan_id = '".$v['id']."'";
			$istatus = $instruction
						->field("status")
						->where($where1)
			...
			{
				$status=array();
				foreach ($istatus as $value) {
					$status[] = $value['status'];
				}
				if(in_array('1',$status) or in_array('0',$status))
				{
					$list[$key]['color'] ='black';
				}else{
					$list[$key]['color'] ='red';
				}
			}else{
			...
			//预报下最新的指令信息
			$ins = $instruction->where("plan_id=" . $v ['id'])->order ( 'id desc' )->find ();
			if ($ins ['location_id'] == null) 
			{
				$list [$key] ['location_name'] = '';
			} else {
				$location = new \Common\Model\LocationModel();
				$res_l = $location->where ( "id=" . $ins ['location_id'] )->find ();
				$list [$key] ['location_name'] = $res_l ['location_name'];
			}
			//指令日期
			$list [$key] ['instruction_date'] = $ins ['ordertime'];
			//装箱方式
			$list [$key] ['operation_method'] = $ins ['loadingtype'];
			//指令条数
			$list [$key] ['ins_count']=$instruction->where("plan_id=" . $v['id'] )->count();
			// 查询预报下的配货信息
			$cargo_sql = "select billno from tally_qbzx_plan_cargo where plan_id = {$v['id']}";
			$cargo = M()->query($cargo_sql);
			$list[$key]['blno'] = $cargo[0]['billno']?$cargo[0]['billno']:"无";
		}
		...
		// {
		// 	//已配箱数
		// 	$sql = "select count(c.id) as ctn_num,i.status istatus from __PREFIX__qbzx_instruction i,__PREFIX__qbzx_instruction_ctn c where i.plan_id='" . $list [$i] ['id'] . "' and c.instruction_id=i.id and c.status!='-1'";
		// 	$res = M ()->query ( $sql );
		// 	$list [$i] ['has_container_num'] = $res [0] ['ctn_num'];
		// 	$list [$i] ['istatus']  = $res[0]['istatus'];
		// 	//预报下最新的指令信息
		...
		// 	// 查询预报下的配货信息
		// 	$cargo_sql = "select billno from tally_qbzx_plan_cargo where plan_id = {$list[$i]['id']}";
		// 	$cargo = M()->query($cargo_sql);
		// 	$list[$i]['blno'] = $cargo[0]['billno']?$cargo[0]['billno']:"无";
		// }
