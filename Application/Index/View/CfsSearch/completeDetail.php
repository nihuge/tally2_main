				...
				<div class="amsg">
					<p>箱号：{$msg['ctnno']}
						...
						if($n>0)
						{
							for($i=0;$i<$n;$i++)
							{
							    echo '<span><a layer-href=".'.IMAGE_CFS_EMPTY.$emptylist[$i] ['empty_img'].'" class="" rel="empty"><span class="showphoto">';
								if($i == 0)
								{
								   echo '( 查看空箱照片 )';
								}
                        ...
                        {
							echo '<span>（<a layer-href=".'.IMAGE_CFS_SEAL.$operationMsg ['seal_picture'].'" class=""><span class="showphoto">查看铅封照片</span></a>）</span>';
						}
						?>&nbsp;&nbsp;&nbsp;&nbsp; 
						<?php
						if ($operationMsg ['halfclose_door_picture']) 
                        {
							echo '<span>（<a layer-href=".'.IMAGE_CFS_HALFCLOSEDOOR.$operationMsg ['halfclose_door_picture'].'" class=""><span class="showphoto">查看半关门照片</span></a>）</span>';
						}
						?>
						&nbsp;&nbsp;&nbsp;&nbsp; 
						<?php
                        ...
                        {
							echo '<span>（<a layer-href=".'.IMAGE_CFS_CLOSEDOOR.$operationMsg ['close_door_picture'].'" class=""><span class="showphoto">查看全关门照片</span></a>）</span>';
						}
						?>
					</p>
					<p>
								...
								<tr>
									...
									<?php
										...
										if ($n > 0) {
											echo '（';
											for($i = 0; $i < $n; $i ++) {
												echo '<span><a layer-href=".'.IMAGE_CFS_CARGO. $l ['level_cargo_img'] [$i] ['level_img'].'" class="" rel="gallery' . $l ['level_num'] . '"><span class="showphoto">';
												if ($i == 0) {
													echo '查看照片';
												}
												echo '</span></a></span>';
									...
									if ($n > 0) {
										echo '（';
										for($i = 0; $i < $n; $i ++) {
											echo '<span><a layer-href=".'.IMAGE_CFS_CDAMAGE.$l ['cargo_damage_img'] [$i] ['img'].'" class="" rel="gallery' . $l ['level_num'] . '"><span class="showphoto">';
											if ($i == 0) {
												echo '查看残损照片';
											}
											echo '</span></a></span>';
							...
							<tbody>
								...
								<tr>
									...
									if (strpos ( $vo ['field_old_value'], '/AppUpload' )) {
										echo '<a layer-href="' . $vo ['field_old_value'] . '" class=""><span class="showphoto">查看原照片</span></a>';
									} else {
									...
									if (strpos ( $vo ['field_new_value'], '/AppUpload' )) {
										echo '<a layer-href="' . $vo ['field_new_value'] . '" class=""><span class="showphoto">查看新照片</span></a>';
									} else {
