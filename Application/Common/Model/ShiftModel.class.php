		...
		if ($res_s ['code'] != 0) {
			...
			if ($res_m ['code'] != 0) {
				...
				foreach ( $res_repair as $key => $r ) {
					// 指令ID
					$instruction_id = $r['instruction_id'];
					\Think\Log::record('instruction_id='.$instruction_id.'business='.$r['business']);
					// 区分业务系统
					switch ($r['business']) {
						case 'qbzx' :
							// 获取所有指令下面的配箱及配箱装箱、状态
						...
						case 'cfs' :
							//
							$CfsInstruction = new \Common\Model\CfsInstructionCtnModel();
							$instruction_ctn = $CfsInstruction->where ( "instruction_id='$instruction_id'" )->select();
							\Think\Log::record('instruction_id='.$instruction_id);
							//判断指令下的工作中的配箱是否暂停作业
							foreach($instruction_ctn as $v1)
							{
								if($v1['status'] == '1')
								{
									$operation = new \Common\Model\CfsOperationModel();
									$res_r = $operation->where("ctn_id='".$v1['id']."'")->field('is_stop')->find();	
									if($res_r['is_stop'] != 'Y')
									{
										$res = array(
												'code'  =>  "211",
