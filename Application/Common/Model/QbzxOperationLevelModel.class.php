...
class QbzxOperationLevelModel extends Model
{
	...
	{
		$sql = "select l.*,u.user_name,p.port_id,p.pack,p.mark from __PREFIX__qbzx_operation_level l,__PREFIX__user u,__PREFIX__qbzx_plan_cargo p where l.operation_id='$operation_id' and l.operator_id=u.uid and p.billno=l.billno group by l.id";
		$list = M ()->query ( $sql );
		if ($list !== false) 
		{
			$num = count ( $list );
	 ...
	 * @return array|boolean
	 */
	public function getLevelListByqbzx($operation_id) 
	{
		$sql = "select l.*,u.user_name from __PREFIX__qbzx_operation_level l,__PREFIX__user u where l.operation_id='$operation_id' and l.operator_id=u.uid group by l.id";
		$list = M ()->query ( $sql );
		if ($list !== false) 
		{
			$num = count ( $list );
