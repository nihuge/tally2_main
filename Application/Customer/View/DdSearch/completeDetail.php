		...
		<div class="right">
			<div class="right_top" style="background: none; border: 0;">
				<div class="right_l" style="font-size: 16px;">
					当前位置：<a href="__MODULE__/DdSearch/real_time">拆箱查询</a>&nbsp;&gt;&nbsp;<a href="__MODULE__/DdSearch/complete">完成作业查询</a>&nbsp;&gt;&nbsp;完成作业详情
				</div>
			</div>
			...
			<div class="right_t">
				<div class="amsg">
					<p>箱号：{$msg['ctn_no']}
                        ...
                        {
							echo '<span>（<a layer-href=".'.IMAGE_DD_CARGO.$operationMsg ['cargo_picture'].'" class=""><span class="showphoto">查看整体货物照片</span></a>）</span>';
						}
						?>
						<?php
						...
						<?php
							...
							if ($n > 0)
                            {
                            	...
                            	for($i = 0; $i < $n; $i ++)
                                {
                                	echo '<a layer-href=".'.IMAGE_DD_DAMAGE. $operationMsg['ctn_damage_img'] [$i] ['img'].'" class="" rel="ctn_damage_img">';
                                	if ($i == 0)
                                    {
                                    	echo '<span class="showphoto">查看箱残损照片</span>';
                                    }
                                    echo '</a>';
                                }
                                echo '）</span>';
						...
						<?php
							if(in_array('1',$photoauth)){
								...
								if ($n > 0)
								{
									...
									for($i = 0; $i < $n; $i ++)
									{
										echo '<a layer-href=".'.IMAGE_DD_DAMAGEAFTER. $operationMsg['ctn_damage_after_img'] [$i] ['img'].'" class="" rel="ctn_damage_after_img">';
										if ($i == 0)
										{
											echo '<span class="showphoto">查看箱残损照片';
										}
										echo '</a>';
									}
									echo '）</span>';
									...
									if(count($levellist) < 3){
										if($a || $b){
											if ($n > 0) {
												echo '（';
												for($j = 0; $j < $n; $j ++) {
													echo '<a layer-href=".'.IMAGE_DD_CARGO.$l ['cargo_level_img'] [$j] ['level_img'].'" class="" rel="gallery' . $l ['level_num'] . '">';
													if ($j == 0) {
														echo '<span class="showphoto">查看货物照片</span>';
													}
													echo '</a>';
												}
												echo '）';
									...
									if(count($levellist) < 3){
										if($a || $b){
											if ($n > 0) {
												echo '（';
												for($j = 0; $j < $n; $j ++) {
													echo '<a layer-href=".'.IMAGE_DD_CDAMAGE.$l ['cargo_damage_img'] [$j] ['img'].'" class="" rel="gallery' . $l ['level_num'] . '">';
													if ($j == 0) {
