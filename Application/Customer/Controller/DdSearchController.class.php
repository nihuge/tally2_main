...
class DdSearchController extends CommonController
{
	...
	public function real_time()
	{
		...
		if($plan_ids){
			...
			$ids = '('.implode(',',$arr).')';
			$where="p.examine='N' and c.plan_id in $ids";
			if(I('get.vslname'))
			{
				$vslname=I('get.vslname');
				$vslname = str_replace("'", "", $vslname);
			...
			$begin_num=($p-1)*$per;
			$sql="select p.vslname,p.voyage,p.unpackagingplace,c.* from __PREFIX__dd_plan p,__PREFIX__dd_plan_container c,__PREFIX__dd_operation o where $where and p.id=c.plan_id and o.ctn_id = c.id order by c.id desc limit $begin_num,$per";
			$list=M()->query($sql);
			//遍历结果，取出其它数据
			$num=count($list);
	...
	public function complete()
	{
		...
		if($plan_ids){
			...
			$ids = '('.implode(',',$arr).')';
			$where = "c.plan_id in $ids and pl.examine='Y'";
			if(I('get.ship_name'))
			{
			...
			->join("tally_dd_plan pl on pl.id=c.plan_id")
			->where($where)->count();
			$per = 15;
			if($_GET['p'])
			{
