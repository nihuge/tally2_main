...
class QbzxSearchController extends CommonController
{
    ...
    public function RealTime()
    {
        ...
        $where = "p.examine='N' and p.entrust_company=$id";
        if (I('get.ship_name')) {
        ...
        $begin_num = ($p - 1) * $per;
        $sql = "select i.plan_id,c.*,u.user_name,p.voyage,s.ship_name,l.location_name
			from tally_qbzx_instruction_ctn c
			LEFT JOIN tally_qbzx_instruction i on i.id=c.instruction_id
    ...
    public function OperationFinish()
    {
        ...
        $id = $_SESSION['id'];
        $where = "pl.entrust_company=$id and pl.examine='Y'";
        if (I('get.ship_name')) {
            $ship_name = I('get.ship_name');
            // 根据船名获取船ID
    ...
    public function ProveByTicket()
    {
        ...
        $id = $_SESSION['id'];
        $where = "p.entrust_company=$id and p.examine='Y'";
        if (I('get.ship_name')) {
            $ship_name = I('get.ship_name');
            // 根据船名获取船ID
            $res_s = $ship->where("ship_name='$ship_name'")->field('id')->find();
