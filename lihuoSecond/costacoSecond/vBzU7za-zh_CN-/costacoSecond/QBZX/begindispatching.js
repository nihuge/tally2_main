define(function(require) {
	require("$UI/costacoSecond/ajax/jquery-1");
	var justep = require("$UI/system/lib/justep");
	var Baas = require("$UI/demo/baas/baas");

	var Model = function() {
		this.callParent();
		this.repair_id;
		this.is_must;
	};

	// orderData的自定义刷新事件onCustomRefresh

	Model.prototype.staffDataCustomRefresh = function(event) {
		var me = this
		var uid = localStorage.getItem('userinfoid');

		var dispatching_gand_id = localStorage.getItem('dispatching_gand_id');
		var dispatching_instruction_id = localStorage.getItem('dispatching_instruction_id');
		var staffData = this.comp("staffData");
		if (debugMode) {
			alert("获取签入同一工班的人员列表接口：" + "\n工班id:" + dispatching_gand_id);
		}
		$.ajax({
			"type" : "post",
			"async" : false,
			"data" : {
				"shift_id" : dispatching_gand_id,

			},
			"dataType" : dataType,
			// ?c=QbzxDispatch&a=addTask
			"url" : interface_url + "?c=QbzxDispatch&a=getWorkList",
			"success" : function(data) {
				if (PrintReturnValue) {
					alert(data);
					return;
				}
				if (data['code'] == 0) {

					staffData.loadData(data['list']);

				} else {

					alert(data['msg']);

				}

			},
			"error" : function() {
				alert("数据传输失败！");
			}

		});

	};

	// 确认派工
	Model.prototype.dispatchingBtn = function(event) {
		var me = this;
		var staffData = this.comp('staffData');
		var orderrepairid = staffData.val('orderrepairid');
		var instruction_id = localStorage.getItem('dispatching');
		var dispatching_gand_id = localStorage.getItem('dispatching_gand_id');
		var uid = localStorage.getItem('userinfoid');
		if (debugMode) {
			alert("新增派工接口" + "\n指令id：" + instruction_id + "\n用户id：" + uid + "\n工班id：" + dispatching_gand_id);
		}
		$.ajax({
			cache : true,
			dataType : dataType,
			// http://192.168.1.91/xztally/app.php/task/tasking?orderrepairid=919&insid=74&uid=20&users[20]
			url : interface_url + "?c=QbzxDispatch&a=addTask" + "&instruction_id=" + instruction_id + "&shift_id=" + dispatching_gand_id + "&chieftally=" + uid,
			data : $('#qbzx_form').serialize(),
			async : false,
			error : function(data) {
				alert("数据传输失败！");
			},
			success : function(data) {
				if (PrintReturnValue) {
					alert(data);
					return;
				}

				if (data['code'] == 0) {
					justep.Shell.showPage("DispatchList");
				} else {
					alert(data['msg']);
				}

				//  
			}
		});
		// $('#form1').submit();
	};

	// 返回键
	Model.prototype.backBtnClick = function(event) {
		justep.Shell.showPage("DispatchList");
	};

	return Model;
});