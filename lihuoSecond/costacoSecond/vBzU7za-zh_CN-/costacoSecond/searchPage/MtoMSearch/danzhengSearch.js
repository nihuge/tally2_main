define(function(require) {
	...
	Model.prototype.shipInfoCustomRefresh = function(event) {
		...
		$.ajax({
			...
			"success" : function(data) {
				...
				$("#dd_danzheng_shipname").autocomplete({
					source : ret
				});
			},
			"error" : function() {
	...
	Model.prototype.searchClick = function(event) {
		...
		var shipname = $("#dd_danzheng_shipname").val();
		...
		$.ajax({
			...
			"url" : interface_url + "?c=DdSearch&a=document", // PHP数据库校验用户名和密码是否正常
			"success" : function(data) {
				if (data['code'] == 0) {
	...
	Model.prototype.li1Click = function(event) {
		...
		justep.Shell.showPage("danzhengDetails_dd");
